...
contract UniswapV2Router02 is IUniswapV2Router02 {
    ...
    function swapExactTokensForTokensSupportingFeeOnTransferTokens(
        ...
        uint balanceBefore = IERC20Upgradeable(path[path.length - 1]).balanceOf(to);
        _swapSupportingFeeOnTransferTokens(path, to);
        require(
            IERC20Upgradeable(path[path.length - 1]).balanceOf(to).sub(balanceBefore) >= amountOutMin,
        ...
        ensure(deadline)
    {
        ...
        uint amountOut = IERC20Upgradeable(WETH).balanceOf(address(this));
